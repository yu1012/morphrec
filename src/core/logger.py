...
class Logger(object):
    ...
    def update_images(self, imgs, key='img'):
        if self.counter.get(key) is None:
            self.counter[key] = 1
        else:
            self.counter[key] += 1        
        ...
        if self.writer:
            for i, img in enumerate(imgs):
                self.writer.add_image(f'sample_{i}', img, self.counter[key])

def build_logger(log_path, cfg):
    if cfg.LOGGER.USE_LOGGER:
        _logger = Logger(tensorboard=cfg.LOGGER.USE_TENSORBOARD,
                         matplotlib=cfg.LOGGER.USE_MATPLOTLIB,
                         log_dir=log_path)
    else:
        _logger = None
